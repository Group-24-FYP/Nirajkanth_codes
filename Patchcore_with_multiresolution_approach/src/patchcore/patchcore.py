...
class PatchCore(torch.nn.Module):
    ...
    def anoscorefit(self, features):
        self.anomaly_scorer.fit(detection_features=[features])

    def predict(self, data):
        if isinstance(data[0], torch.utils.data.DataLoader):
    ...
    def _predict_dataloader(self, dataloader):
        ...
        dataloader1, dataloader2, dataloader3 = dataloader
        #with tqdm.tqdm( dataloader, desc="Inferring...", leave=False) as data_iterator: 
        
            #for images in zip(dataloader1, dataloader2, dataloader3):
        for image1, image2, image3 in tqdm.tqdm(zip(dataloader1, dataloader2, dataloader3), desc="Inferring...", leave=False):# process batches
                #print(images)
                if isinstance(image1, dict):
                    labels_gt.extend(image1["is_anomaly"].numpy().tolist())
                    mask =  image1['mask']
                    masks_gt.extend(mask.numpy().tolist())
                    image1 = image1["image"]

                    mask =  image2['mask']
                   # masks_gt['maskgt2'].extend(mask.numpy().tolist())
                    image2 = image2["image"]

                    mask =  image3['mask']
                   # masks_gt['maskgt3'].extend(mask.numpy().tolist())
                    image3 = image3["image"]
                    #print(image)
                #for i in range(len(images)):
                    #_scores, _masks = self._predict(image)
                    batchsize = image1.shape[0]
                    features1, patch_shapes1  =  self._predict(image1)
                    features2, patch_shapes2  =  self._predict(image2)
                    features3, patch_shapes3  =  self._predict(image3)
                    
                    #f_all = np.concatenate((features1, features2, features3), axis=0)
                    f_all = np.expand_dims( np.mean((features1, features2, features3), axis=0), axis=0)
                
                    #torch.cuda.empty_cache()

                    patch_scores, query_distances, query_nns = self.anomaly_scorer.predict([f_all])
                    ##patch_scores = image_scores = self.anomaly_scorer.predict([features])[0]
                    
                    image_scores = patch_scores 
                    #print("patch_scores", patch_scores, patch_scores.max(), patch_scores.shape)
                    ...
                    #print("im_scores", image_scores, image_scores.shape)
                    ...
                    scales = patch_shapes2[0]
                    patch_scores = patch_scores.reshape(batchsize,scales[0], scales[1])
                    #patch_scores = patch_scores.reshape( 1 , scales[0], scales[1])
                    #print("patch_scores", patch_scores, patch_scores.shape)

                    masks1 = self.anomaly_segmentor.convert_to_segmentation(patch_scores)
                    
                    _scores, _masks = [score for score in image_scores], [mask for mask in masks1]
                    #print(_masks)
    ...
    def _predict(self, images):
        ...
        with torch.no_grad():
            ...
            print('embed freat', features.shape)
            #print('ps', patch_shapes )
