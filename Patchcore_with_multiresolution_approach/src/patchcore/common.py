...
class FaissNN(object):
    ...
    def fit(self, features: np.ndarray) -> None:
        ...
        print('ntotal',self.search_index.ntotal)
    ...
    def run(
        ...
        if index_features is None:
            #print("yes", self.search_index.ntotal, query_features.shape)
        ...
        #print('search index', search_index.ntotal)
...
class ConcatMerger(_BaseMerger):
    ...
    def _reduce(features):
        ...
        return features.reshape(-1, np.array(features).shape[-1])
...
class Preprocessing(torch.nn.Module):
    def __init__(self, input_dims, output_dim):
        ...
        #print('input dim', input_dims)
        #print('output dim', output_dim)

        self.preprocessing_modules = torch.nn.ModuleList()
        for input_dim in input_dims:
            module = MeanMapper(output_dim)
            #print('module', module.shape)
            self.preprocessing_modules.append(module)

    def forward(self, features):
        _features = []
        for module, feature in zip(self.preprocessing_modules, features):
            #print('fp', feature.shape)
            #print('fp1', module(feature).shape)
...
class MeanMapper(torch.nn.Module):
    ...
    def forward(self, features):
        #print('mean map1',features.shape)
        features = features.reshape(len(features), 1, -1)
        #print('mean map2',features.shape)
        #print('mm final', F.adaptive_avg_pool1d(features, self.preprocessing_dim).squeeze(1).shape, self.preprocessing_dim)
...
class Aggregator(torch.nn.Module):
    ...
    def forward(self, features):
        ...
        features = features.reshape(len(features), 1, -1)
        #print('a f', features.shape)
        features = F.adaptive_avg_pool1d(features, self.target_dim)
        #print('af ada', features.shape)
        return features.reshape(len(features), -1)
    ...
    def convert_to_segmentation(self, patch_scores):
        ...
        with torch.no_grad():
            ...
            _scores = F.interpolate(
                _scores, size=self.target_size, mode="bilinear", align_corners=True
            )
            #_scores = F.interpolate(
            #    _scores, size=224, mode="bilinear", align_corners=False
            #)
...
class NetworkFeatureAggregator(torch.nn.Module):
    ...
    def __init__(self, backbone, layers_to_extract_from, device):
        ...
        #for extract_layer in [ '7', '8']:
            #################### 3-changes #############
            forward_hook = ForwardHook( self.outputs, extract_layer, layers_to_extract_from[-1])
            #forward_hook = ForwardHook(self.outputs,  extract_layer, '8')
            if "." in extract_layer:
                extract_block, extract_idx = extract_layer.split(".")
                network_layer = backbone.__dict__["_modules"][extract_block]
                #network_layer = backbone.__dict__["_modules"][extract_block][0]
                if extract_idx.isnumeric():
                    extract_idx = int(extract_idx)
                    network_layer = network_layer[extract_idx]
                    #network_layer = network_layer[extract_idx][0]
                else:
                    network_layer = network_layer.__dict__["_modules"][extract_idx]
                    #network_layer = network_layer.__dict__["_modules"][extract_idx][0]
            else:
                network_layer = backbone.__dict__["_modules"][extract_layer]
                #network_layer = backbone.__dict__["_modules"][extract_layer][0]
                #network_layer = backbone.__dict__["_modules"]['blocks'][eval(extract_layer)]
                print(f"{extract_layer} extracted from pretrained model")
    ...
    def feature_dimensions(self, input_shape):
        ...
        #return [_output[layer].shape[1] for layer in [ '7', '8']]
...
class NearestNeighbourScorer(object):
    def __init__(self, n_nearest_neighbours: int, nn_method=FaissNN(False, 4)) -> None:
        ...
        self.imagelevel_nn = lambda nn, query: self.nn_method.run(nn,query)
    ...
    def fit(self, detection_features: List[np.ndarray]) -> None:
        ...
        Args:
            detection_features: [list of np.arrays]
                ...
                anomaly detection.
        """
        #print('detection features1', detection_features.shape)
        self.detection_features = self.feature_merger.merge(
            detection_features,
        )
        #self.detection_features = np.array(detection_features)
        print('detection features', self.detection_features.shape)
    ...
    def predict(
        ...
        """
        print('q f', query_features[0].shape)
        query_features = self.feature_merger.merge(
            query_features,
        )
        #print('q feat', query_features.shape)
        nn = 1
        query_distances, query_nns = self.imagelevel_nn(nn, query_features)
        ...
        def softmax(x,k):
            ...
            #print(e_x)
            e_y = np.array([e_x[:,:k].sum(axis=1), e_x[:,-nn+k:].sum(axis=1)]).T
            #print(e_y)
            return e_y/np.sum(e_x, axis=1, keepdims=True)
        k = nn - 1
        #softmax_value = softmax(query_distances, k)
