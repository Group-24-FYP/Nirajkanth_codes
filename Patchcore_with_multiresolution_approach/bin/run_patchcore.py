    ...
    #for dataloader1, dataloader2, dataloader3 in zip(list_of_dataloaders1, list_of_dataloaders2, list_of_dataloaders3):
        ...
        with device_context:
            ...
            for i, PatchCore in enumerate(PatchCore_list):
                ...
                if i == 0:
                    f1 = PatchCore.fit(list_of_dataloaders1[j]["training"])
                    print('f1', f1.shape)
                    f_all.append(np.expand_dims(f1, axis=0))
                elif i == 1:
                    f2 = PatchCore.fit(list_of_dataloaders2[j]["training"])
                    print('f2', f2.shape)
                    f_all.append(np.expand_dims(f2, axis=0))
                elif i == 2:
                    f3 = PatchCore.fit(list_of_dataloaders3[j]["training"])
                    #print('f3', type(f3), f3.shape)
                    print('f3', f3.shape)
                    f_all.append(np.expand_dims(f3, axis=0))

            f_all = np.array(f_all).mean(axis=0)
            ...
            for i, PatchCore in enumerate(PatchCore_list):
                ...
                if i == 1:
                    scores, segmentations, labels_gt, masks_gt = PatchCore.predict([list_of_dataloaders1[j]["testing"],
                                                                                    list_of_dataloaders2[j]["testing"],
                                                                                    list_of_dataloaders3[j]["testing"]])
                    #scores, segmentations, labels_gt, masks_gt = PatchCore.predict(list_of_dataloaders1[j]["testing"])                                                                
            ...
            anomaly_labels = [
                x[1] != "good" for x in list_of_dataloaders3[j]["testing"].dataset.data_to_iterate
            ]
            ...
            if save_segmentation_images:
                ...
                mask_paths = [
                    x[3] for x in list_of_dataloaders2[j]["testing"].dataset.data_to_iterate
                ]
            ...
            aupr = patchcore.metrics.compute_imagewise_retrieval_metrics(
                scores, anomaly_labels
            )["aupr"] 
            ####auroc, aupr = 0.5, 0.5
            ################
            ...
            pixel_scores_aupr = patchcore.metrics.compute_pixelwise_retrieval_metrics(
                [segmentations[i] for i in sel_idxs_aupr ], [masks_gt[i] for i in sel_idxs_aupr ]
            )
            ...
            if log_online:
                wandb.init(project="VISA-patchcore-niraj-multires", entity="fyp_anomaly_detection")
                wandb.log({
                    "dataset_name": dataset_name,
                    'layers' : '3wres50', 
                    'imagesize' : imagesize,
